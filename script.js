...
let autor = document.getElementById("auteur");
let nou = document.getElementById("nouveau");
...
function generateIntNumbers(max) {
  return Math.floor(Math.random() * Math.floor(max));
}

nou.addEventListener("click", () => {
  do {
    randomNumbers = generateIntNumbers(citations.length);
  } while (randomNumbers == dernier);

  quote.textContent = citations[randomNumbers][0];
  autor.textContent = citations[randomNumbers][1];
  dernier = randomNumbers;
});
